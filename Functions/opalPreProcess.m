...
%-------------------------------------------------------------------------%
    ...
    recordIdx = find(startsWith(string(combinedCsv.Properties.VariableNames),"Record"));
    idx = combinedCsv.Properties.VariableNames(recordIdx);

    combinedCsv = sortrows(combinedCsv,idx,"ascend"); 
    ...
    for t = 1:height(combinedCsv)
        ...
        if startsWith('MDT',combinedCsv.(string(idx)){t}(end-2:end)) || startsWith('MST',combinedCsv.(string(idx)){t}(end-2:end))
            ...
            time = datetime({combinedCsv.(string(idx)){t}(1:15)}, 'InputFormat', 'yyyyMMdd-HHmmss','TimeZone','UTC','Format','HH:mm:ss');  
        ...
        for i = 1:length(raw.sensor)
            if strncmp(rrData.sensor(i).monitorLabel,"Forehead",4)
                sens = "head";    
                opal.(string(combinedCsv.Condition(t))).(sens).acc = [rrData.sensor(i).acc.x' rrData.sensor(i).acc.y' rrData.sensor(i).acc.z'];
                opal.(string(combinedCsv.Condition(t))).(sens).gyro = [rrData.sensor(i).gyro.x' rrData.sensor(i).gyro.y' rrData.sensor(i).gyro.z'];
