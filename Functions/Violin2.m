classdef Violin2 < handle
    ...
    properties (Dependent=true)
        ...
        ShowNotches % whether to show notch indicators
        ShowMean    % whether to show mean indicator
        Sides       % whether to display both sides or only left or right % Added by Peter Fino 3/6/2020
    end
    
    methods
        function obj = Violin2(data, pos, varargin)
            ...
            %     'ShowMean'     Whether to show mean indicator.
            %                    Defaults to false
            %     'Sides'        Indicates which sides of the violin show
            %                    Defaults to Both
            ...
            if length(density) > 1
                jitterstrength = 0; %  interp1(value, density*width, data);
...
%                 scatter(pos + jitter.*jitterstrength, data, 'filled');
            ...
            % plot the violin
            if strmatch(args.Sides, 'Right')
                pos = pos + 0.05;
                jitter = 2*(rand(size(data))-0.5);
                obj.ScatterPlot = ...
                    scatter(pos - 0.025 + abs(jitter.*jitterstrength), data, 'filled');
                obj.ViolinPlot =  ... % plot color will be overwritten later
                    fill([pos+density*width pos.*ones(1,length(density(end:-1:1)))], ...
                    [value value(end:-1:1)], [1 1 1]);
            else if strmatch(args.Sides, 'Left')
                    pos = pos - 0.05;
                    jitter = 2*(rand(size(data))-0.5);
                    obj.ScatterPlot = ...
                        scatter(pos + 0.025 + abs(jitter.*jitterstrength), data, 'filled'); % - instead of + originally
                    obj.ViolinPlot =  ... % plot color will be overwritten later
                        fill([pos-density*width pos.*ones(1,length(density(end:-1:1)))], ...
                        [value value(end:-1:1)], [1 1 1]);
                else
                    jitter = 2*(rand(size(data))-0.5);
                    obj.ScatterPlot = ...
                        scatter(pos - 0.05 + jitter.*jitterstrength, data, 'filled');
                    obj.ViolinPlot =  ... % plot color will be overwritten later
