...
fprintf('Mean Weekend NumTurns: m = %.2f (%.2f)\n', mean(weekendTurns), std(weekendTurns))
fprintf('Mean Weekday NumTurns: m = %.2f (%.2f)\n', mean(weekdayTurns), std(weekdayTurns))

weekendAmp = T.Mean(weekendIdx);
weekdayAmp = T.Mean(weekdayIdx);
fprintf('Mean Weekend Amplitude: m = %.4f (%.2f)\n', mean(weekendAmp),std(weekendAmp))
fprintf('Mean Weekday Amplitude: m = %.4f (%.2f)\n', mean(weekdayAmp),std(weekdayAmp))
