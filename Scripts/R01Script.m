...
for s = 1:length(subjects)
    ...
    folderVariable = fieldnames(interest.(subjects{s}));
...
tasks = fieldnames(crossFuzzyStatResultant);
for tt = 1:length(tasks)
        varName = append(tasks{tt},'_Resultant');
        results.(varName) = crossFuzzyStatResultant.(tasks{tt})';
end
 
tasks = fieldnames(crossFuzzyStat);
direction = {'AP','ML','Vert'};
for tt = 1:length(tasks)
    for dd = 1:length(direction)
        varName = append(tasks{tt},'_',direction{dd});
        results.(varName) = crossFuzzyStat.(tasks{tt})(:,dd);
    end
end
...
%% run pair-wise effect sizes for the resultant using hedge's g? (use the mes function in matlab) 

clc
unique_groups = unique(results.Type);

% Compute pairwise effect sizes
for i = 1:length(unique_groups)
    for j = i+1:length(unique_groups)
        % Get indices for each group
        % task = results.YOYO_Resultant;
        task = results.Buffalo_Resultant;
        group_i = task(results.Type== unique_groups(i));
        group_j = task(results.Type == unique_groups(j));

        % Compute Hedge's g
        effectName = append('group',num2str(i-1),num2str(j-1));
        effect.(effectName) = mes(group_i, group_j, 'hedgesg');

        % Display results
        fprintf('Hedges g (Group %d vs Group %d): %.3f, 95%% CI: [%.3f, %.3f]\n', ...
            unique_groups(i), unique_groups(j), effect.(effectName).hedgesg, effect.(effectName).hedgesgCi(1), effect.(effectName).hedgesgCi(2));
    end
end

%% Figure CFuzzyEn for resultant

variable = fieldnames(results);
% xx = listdlg('PromptString',{'Select X Variable',''},...
%         'SelectionMode','single','ListString',variable);
yy = listdlg('PromptString',{'Select Y Variable',''},...
        'SelectionMode','single','ListString',variable);

% Define jitter amount
jitterAmount = 0.2; % Adjust as needed

% Generate jittered X values
xValues = results.Type; % Assuming Type is categorical
xJitter = xValues + jitterAmount * (rand(size(xValues)) - 0.5); 

figure
gscatter(xJitter, results.(variable{yy}), results.Type)
xlabel('Type (Jittered)')
ylabel(variable{yy})
title('Jittered gscatter Plot')
saveas(gcf,variable{yy},'svg')

%% Descriptive Statistics

variable = fieldnames(results);
yy = listdlg('PromptString',{'Select Y Variable',''},...
        'SelectionMode','multiple','ListString',variable);

for y = 1:length(yy)
    unique_groups = unique(results.Type);
    for i = 1:length(unique_groups)
        varMean = mean(results.(variable{yy(y)})(results.Type == unique_groups(i)));
        varStd = std(results.(variable{yy(y)})(results.Type == unique_groups(i)));
        effectName = append('group',num2str(i-1));
        desStat.(effectName).(variable{yy(y)}) = [varMean varStd];
