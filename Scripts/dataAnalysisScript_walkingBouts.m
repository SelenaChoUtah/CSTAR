...
% C:\Users\chose\Box\H&K Fino Lab\Older Adult Concussion - PilotS
    ...
    try
        ...
        for j = 1:length(daynum)  
            ...
            for s = 1:length(sensor)
                ...
                % 
                % % Rotating Accelerometer
                % [A_data2,Rot_data2] = rotateIMU(accData,4,40,100,fullWindow,calibrateWindow);
                % % Rotate Gyroscope
                % [G_data2] = rotateGyro(gyroData,Rot_data2,order,Fc,Fs,fullWindow,calibrateWindow);
                % 
                ...
                % 
                % amplitudeThreshold = 10; % deg minimum amplitude for head turn
                % velocityThreshold = 10; % deg/s peak velocity to quantify as turn
                % minima = 5; % Local Minima     
                % impulseDuration = 0.2; % Larger value means more smoothed
            ...
            %% Head Turn Metrics During Walking bouts
            
            sensor = fieldnames(stepData.(daynum{j}));
            if isfield(stepData.(daynum{j}), 'waist') && isfield(stepData.(daynum{j}), 'head')
                gyro2 = rotate.(id).(daynum{j}).head.gyro(:,3);
                impulseDuration = 1.476;
                filterData = ShahFilter(gyro2,impulseDuration,100); 
                walkWindow = stepData.(daynum{j}).waist.walkBouts;
                % turns during walk window
                for ff = 1:length(walkWindow)-1
                    if walkWindow(ff,2) > length(filterData)
                        continue
                    else
                        if all(structfun(@isempty,absShahTurn(filterData(walkWindow(ff,1):walkWindow(ff,2)),gyro2(walkWindow(ff,1):walkWindow(ff,2)),minima,amplitudeThreshold,velocityThreshold,impulseDuration)))
                            continue
                        else
                            walkBout = sprintf('w%d', ff);
                            turnWalk.(id).(walkBout) = absShahTurn(filterData(walkWindow(ff,1):walkWindow(ff,2)),gyro2(walkWindow(ff,1):walkWindow(ff,2)),minima,amplitudeThreshold,velocityThreshold,impulseDuration);   
                        end
                    end
                end                
                saveData.turnWalk.(daynum{j}).(sensor{ss}) = turnWalk.(id);
            end
        ...
        try
            % CSTAR
            subIDFold = strcat(currentFoldPath,'\Data\Walking\', id,filesep);
